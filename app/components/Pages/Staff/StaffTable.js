...
import { MoreVertical, Shield, Mail, Key, Loader, Loader2 } from "lucide-react"
...
export default function StaffTable({ searchQuery }) {
  ...
  // fetch staff data from MongoDB and set it to the customer array in the application state.
  useEffect(() => {
    async function fetStaff() {
      setIsFetching(true)
      try {
        const response = await Axios.get(`${appState.backendURL}/get-all-staff`)
        appDispatch({ type: "setStaff", payload: response.data })
        setIsFetching(false)
      } catch (err) {
        ...
        appDispatch({
          ...
          payload: {
            type: "error",
            msg: "There is a problem fetching staff, try again later",
          },
        })
        setIsFetching(false)
      }
